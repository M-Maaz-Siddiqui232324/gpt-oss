#!/usr/bin/env python3
"""
Sequential Users Test Script for FlowHCM RAG Chatbot
Simulates users asking questions one by one (sequentially) and stores conversations in database.
"""
...
TEST_RESULTS_FILE = "test_sequential_users_results.json"
SEQUENTIAL_USERS = 10
...
class SequentialUserTester:
    ...
    async def __aenter__(self):
        ...
        connector = aiohttp.TCPConnector(
            limit=10,               # Lower limit for sequential testing
    ...
    async def simulate_user(self, user_id: str, user_name: str, num_queries: int = 6) -> List[Dict[str, Any]]:
        ...
        for i, query in enumerate(user_queries):
            # Add small delay between queries (1-3 seconds)
            if i > 0:
    ...
    async def run_sequential_test(self, num_users: int, queries_per_user: int = 6) -> List[Dict[str, Any]]:
        """Run sequential user simulations (one user at a time)"""
        logger.info(f"🎯 Starting sequential test with {num_users} users, {queries_per_user} queries each")
        ...
        # Run users sequentially (one at a time)
        logger.info("🔄 Running user simulations sequentially...")
        all_results = []
        
        for i, (user_id, user_name) in enumerate(users):
            logger.info(f"👤 Processing user {i+1}/{num_users}: {user_name}")
            
            try:
                user_results = await self.simulate_user(user_id, user_name, queries_per_user)
                all_results.extend(user_results)
                
                # Add delay between users (2-5 seconds)
                ...
                logger.error(f"❌ User {user_name} simulation failed: {e}")
                all_results.append({
                    "success": False,
                    "user_id": user_id,
                    "user_name": user_name,
    ...
    def save_results(self, results: List[Dict[str, Any]], filename: str):
        ...
        test_summary = {
            "test_info": {
                "timestamp": datetime.now().isoformat(),
                "test_type": "Sequential Users Test",
        ...
        print("\n" + "="*80)
        print("🎯 SEQUENTIAL USERS TEST SUMMARY")
        print("="*80)
...
async def main():
    ...
    logger.info("🚀 Starting sequential users test for FlowHCM RAG Chatbot")
    
    async with SequentialUserTester(API_BASE_URL) as tester:
        try:
            # Run the sequential test
            results = await tester.run_sequential_test(SEQUENTIAL_USERS, queries_per_user=6)
            
            # Save results to file
            tester.save_results(results, TEST_RESULTS_FILE)
            
            logger.info("🎉 Sequential users test completed successfully")
